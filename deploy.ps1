...
Write-Host "║   EnterChat - Unified Messaging        ║" -ForegroundColor Cyan
...
if ($Install -or -not (Test-Path "$EnterChatDir\.dart_tool")) {
...
if (-not (Test-Path "$EnterChatDir\.env")) {
    if (Test-Path "$EnterChatDir\.env.example") {
        Copy-Item "$EnterChatDir\.env.example" "$EnterChatDir\.env"
...
switch ($Platform) {
    "android" {
        Write-Info "Building for Android (with Accessibility Service)..."
        if ($Release) {
            flutter build apk --release
            if ($LASTEXITCODE -eq 0) {
                Write-Success "Android APK built successfully"
                Write-Info "APK location: build\app\outputs\flutter-apk\app-release.apk"
                Write-Warning-Custom "Enable Accessibility Service after installation"
            }
        }
        else {
            if ($Run) {
    ...
    "web" {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            # Serve web build
            Write-Host ""
            Write-Info "Starting web server on port 3005..."
            cd build\web
...
Write-Success "EnterChat Deployment Complete"
Write-Host "═══════════════════════════════════════" -ForegroundColor Green
